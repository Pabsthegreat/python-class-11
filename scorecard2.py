...
#menu based input
            print("a)runs b)wicket c)no ball d)wide e)byes,leg-byes f)penalties")

            ball = input("What happened this ball?  :")
            print("\n")

            while len(ball.strip()) == 0 or ball not in ("a","b","c","d","e","f","g"):
...
#printing bat, bowl, runs at end of over            
        print(bat[0],":",sum(bat1[bat[0]]),"(",len(bat1[bat[0]]),")\n",\
            bat[1],":",sum(bat1[bat[1]]),"(",len(bat1[bat[1]]),")""\n",\
                bo1,":",sum(bow[bo1]),"-",wicket[bo1],"\n",runs,"-",wickets)
